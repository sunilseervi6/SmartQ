...
export const joinQueue = async (req, res) => {
  try {
    ...
    if (room.operatingHours && room.operatingHours.start && room.operatingHours.end) {
      const now = new Date();
      ...
      if (currentMinutes < startMinutes || currentMinutes >= endMinutes) {
        return res.status(400).json({
          message: `Queue is closed. Operating hours are ${room.operatingHours.start} - ${room.operatingHours.end}`
        });
      }
    }
    ...
    // Create queue entry (queueNumber is auto-generated by the pre-save hook)
    ...
    if (user.notificationPreferences.queueJoined) {
      ...
      await createNotification(customerId, {
        type: 'queue_joined',
        title: 'Joined Queue',
        message: `You've joined the queue at ${shopData.name} - ${room.name}. Your queue number is ${queueEntry.queueNumber}.`,
...
export const leaveQueue = async (req, res) => {
  try {
    ...
    const roomId = queueEntry.roomId;

    // Hard delete - remove from database completely
    await Queue.findByIdAndDelete(queueId);

    // Emit real-time update so other clients see the change
    const io = req.app.get('io');
    if (io) {
      io.to(`room-${roomId}`).emit('queue-updated', {
        type: 'customer-left',
        roomId
      });
    }
  ...
  try {
    ...
    res.json({
      ...
      queue: queue.map((entry, index) => ({
        ...
        customerName: entry.customerName || entry.customerId.name,
        status: entry.status,
  ...
  try {
    ...
    res.json({
      ...
      customer: {
        id: nextCustomer._id,
        queueNumber: nextCustomer.queueNumber,
        name: nextCustomer.customerName || nextCustomer.customerId.name,
...
export const completeService = async (req, res) => {
  try {
    ...
    const io = req.app.get('io');
    if (io) {
      io.to(`room-${queueEntry.roomId._id}`).emit('queue-updated', {
